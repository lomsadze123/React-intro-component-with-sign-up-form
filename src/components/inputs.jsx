        ...
        <Form onSubmit={handleError}>
            {labelList.map(list => {
                return <div key={list.id}>
                    <label>
                        <Input about={error[list.id-1] ? ".2rem solid #FF7979": ".1rem solid #DEDEDE"} type={list.type} placeholder={list.placeholder}  />
                        <Img src={ErrorIcon} about={error[list.id-1] ? "inline-block" : "none"} alt="Error Icon" />
                    </label>
                    <ErrorP about={error[list.id-1] ? "block" : "none"}>{list.placeholder} cannot be empty</ErrorP>
                </div>
            })}
